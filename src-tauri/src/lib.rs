...
pub struct Anniversaries {
    ...
    next_id: u32, // Track the next ID to use
}
...
fn save_anniversary(app_handle: AppHandle, name: String, date: String) -> Result<(), String> {
    ...
    // Create new anniversary with the next available ID
...
fn delete_anniversary(app_handle: AppHandle, id: u32) -> Result<(), String> {
    println!("Rust backend: Deleting anniversary with ID: {}", id);

    // Load current anniversaries
    let mut anniversaries = load_anniversaries(&app_handle)?;
    println!("Loaded {} anniversaries", anniversaries.items.len());

    // Debug: Print all anniversaries and their IDs
    for (i, anniv) in anniversaries.items.iter().enumerate() {
        println!("Index: {}, ID: {:?}, Name: {}", i, anniv.id, anniv.name);
    }

    // Find the index of the anniversary with the given ID
    let index = anniversaries.items.iter().position(|a| a.id == Some(id));
    println!("Found index for ID {}: {:?}", id, index);

    match index {
        Some(idx) => {
            // Remove the anniversary at the found index
            let removed = anniversaries.items.remove(idx);
            println!("Removed anniversary: {:?}", removed.name);

            // Save the updated anniversaries list
            let app_dir = app_handle
                .path()
                .app_data_dir()
                .map_err(|e| e.to_string())?;
            ...
            fs::write(&anniversaries_path, json).map_err(|e| e.to_string())?;
            println!("Saved updated anniversaries to {:?}", anniversaries_path);

            Ok(())
        }
        None => {
            let err_msg = format!("Anniversary with ID {} not found", id);
            println!("{}", err_msg);
            Err(err_msg)
        }
    }
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            get_anniversaries,
            calculate_date,
            delete_anniversary // Make sure this is included
        ])
